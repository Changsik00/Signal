<template>
  <section style="padding: 50px; ">
    <div>
      <span style="font-size: 20px; font-weight: bold">언급된 이슈어</span> (수집기간 : 전체기간 )
    </div>
    <div style="color: #8a8a8a">키워드에 대한 각각의 언급된 이슈어를 시각화하여 제공해 드립니다.</div>
    <div style="display: flex; width: 1000px; margin: auto">
      <div>
        <div id="bubbleBarChart0"></div>
        <div style="text-align: center; font-size:18px; font-weight: bold">{{ categories[0] }}</div>
      </div>
      <div>
        <div id="bubbleBarChart1"></div>
        <div style="text-align: center; font-size:18px; font-weight: bold">{{ categories[1] }}</div>
      </div>
      <div>
        <div id="bubbleBarChart2"></div>
        <div style="text-align: center; font-size:18px; font-weight: bold">{{ categories[2] }}</div>
      </div>
    </div>
  </section>
</template>
      ...
      node
        .on("mouseover", function(d, i) {
          if (d.r < 50) {
            ...
            setTimeout(() => d3.select(this).attr("transform", d => `translate(${d.x},${d.y}) scale(${50 / d.r})`), 0)
          }
        })
        .on("mouseout", function(d, i) {
          if (d.r < 50) {
            d3.select(this).attr("transform", d => `translate(${d.x},${d.y}) scale(1)`)
