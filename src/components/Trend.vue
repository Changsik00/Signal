      ...
      <div v-for="(d, i) in totalResultTop10" :key="i">
        ...
        >
          ...
          <div style="margin-left: 20px;">
            <div style="font-size: 18px; font-weight: bold;" v-html="d.title"></div>
  ...
  mounted() {
    ...
    this.$axios
      ...
      .then(res => {
        ...
        const top10 = totalResult.sort((a, b) => b.value - a.value).slice(0, 10)
  ...
  methods: {
    ...
    async requestTop10(top10) {
      ...
      top10.forEach(async d => {
        ...
        if (
          ...
          (imageResult.data.image.endsWith("png") || imageResult.data.image.endsWith("jpg"))
        ) {
    ...
    draw(data) {
      const config = {
        ...
        color_range: ["#990000", "#cccc00", "#009900", "#004c99", "#4c0099", "#99004c", "#66b2ff", "#a0a0a0a"],
        itemLabel: "左侧文字",
        typeLabel: "右侧文字",
        item_x: 250,
      ...
      function getColor(d) {
        ...
        if (d[divide_color_by] in config.color) return config.color[d[divide_color_by]]
        else {
