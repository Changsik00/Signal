<template>
  ...
  >
    <v-layout
      column
      style="padding: 0; height: 100%; background-color: white; overflow: hidden"
    >
      ...
      <div style="background-color: white; height: 100%; overflow-y: auto">
        ...
        >
          <div style="font-size: 12px; margin-top: 20px;">
            * 로그인 함으로써
            <a href="/policy/terms" class="login-policy-link">서비스이용약관</a
            >&nbsp
            <a href="/policy/privacy" class="login-policy-link"
              >개인정보처리방침</a
            >에 동의 합니다.
          </div>
          <form class="mt30" @submit.prevent="authenticateEmail">
            ...
            <v-btn type="submit" class="mt30" block color="secondary"
              >로그인</v-btn
            >
          </form>
          <v-layout align-center class="mt20">
            <div class="login-link" @click="showFindPassword = true">
              비밀번호를 잊으셨나요?
            </div>
            <v-spacer></v-spacer>
        ...
        >
          <div class="login-link" @click="showSignUp = false">
            &lt; 로그인으로 이동
          </div>
        ...
        <v-layout
          ...
          style="width: 300px; margin:50px auto;"
        >
          <div class="login-link" @click="showFindPassword = false">
            &lt; 로그인으로 이동
          </div>
          <h3 class="mt30">
            등록된 이메일로 패스워드 변경
