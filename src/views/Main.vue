        ...
        <v-navigation-drawer v-model="$store.state.monitorSlideMenu" absolute temporary right width="320" style="padding-bottom: 20px">
            <v-layout align-center style="padding: 10px 0; background-color: whitesmoke;">
                <div v-if="searchOn" style="display: flex; align-items: center; margin-left: 10px;">
                    <img v-if="searchType == 'NAVER_KEY_WORD'" src="../assets/img/common/naver2-on.svg" style="width: 25px; margin-right: 5px;" />
                    <img v-if="searchType == 'TWITTER_KEY_WORD'" src="../assets/img/common/twitter-on.svg" style="width: 25px; margin-right: 5px;" />
                    <div>News Search</div>
                </div>
                <div v-else style="font-size: 20px; font-weight: bold; margin-left: 15px;">Select a column type</div>
                <v-spacer></v-spacer>
            ...
            <div style="position: relative; display: flex; flex-direction: column;">
                <div v-if="searchOn" class="side-menu">
                    <v-layout v-if="searchKeywords.length < 3" align-center style="width: 100%; padding: 10px">
                    ...
                    <div v-if="searchCheck" class="side-menu line">
                        <div v-for="(feed, i) in searchPreviewList" :key="i" style="padding: 10px;">
                            <div v-if="feed.title" v-html="feed.title" style="font-size: 16px; font-weight: bold;"></div>
                            <div v-if="feed.description" class="mt10" style="font-size: 14px;" v-html="feed.description"></div>
                            ...
                            <div v-if="feed.user" class="mt5" style="font-size: 14px; font-weight: bold;" v-html="feed.user"></div>
                        </div>
                    </div>
                </div>
                <div v-else>
                    ...
                    <div class="side-menu line">
                        <v-layout align-center class="title">
                            <img v-if="$store.state.snsConnect.facebook" src="../assets/img/common/facebook-on.svg" style="width: 25px; margin-right: 5px;" />
                            <img v-else src="../assets/img/common/facebook-off.svg" style="width: 25px; margin-right: 5px;" /> Facebook
                        </v-layout>
                        <div class="menu" @click="facebook('VisitorPosts')">
                            Visitor Posts
                            <span v-if="connectFacebookPosts" style="font-size: 12px; color: #9da6af">(모니터링)</span>
                        </div>
                        <div class="menu" @click="facebook('PageMentions')">
                            Page Mentions
                            <span v-if="connectFacebookMentions" style="font-size: 12px; color: #9da6af">(모니터링)</span>
                        </div>
                    ...
                    <div class="side-menu line">
                        <v-layout align-center class="title">
                            <img v-if="$store.state.snsConnect.twitter" src="../assets/img/common/twitter-on.svg" style="width: 25px; margin-right: 5px;" />
                            <img v-else src="../assets/img/common/twitter-off.svg" style="width: 25px; margin-right: 5px;" /> Twitter
                        </v-layout>
                        <div class="menu" @click="twitter('Mentions')">
                            Mentions
                            <span v-if="connectTwitterMentions" style="font-size: 12px; color: #9da6af">(모니터링)</span>
                        </div>
                        <div class="menu" @click="twitter('Timeline')">
                            Timeline
                            <span v-if="connectTwitterTimeline" style="font-size: 12px; color: #9da6af">(모니터링)</span>
                        </div>
